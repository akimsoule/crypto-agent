...
import prisma from "../../../../../netlify-old/src/infrastructure/database/prismaClient";
import {
  CryptoProject,
  GemCriteria,
  CryptoAlert,
...
export class CryptoGemHunter {
  ...
  constructor() {
    ...
    this.gemCriteria = {
      maxMarketCap: 5000000000, // beaucoup plus large
      minVolumeIncrease: 10, // encore plus permissif
      minPriceIncrease: 1, // encore plus permissif
      maxRank: 3000, // plus large
      minSentimentScore: 0.2, // plus permissif
    };
  }
  ...
  async fetchMarketData(page: number = 1, perPage: number = 250) {
    try {
      ...
      const validCryptos = response.data.filter((crypto: CryptoData) => {
        ...
        const price =
          typeof crypto.quotes.USD.price === "string"
            ? parseFloat(crypto.quotes.USD.price)
            : crypto.quotes.USD.price;
        const marketCap =
          typeof crypto.quotes.USD.market_cap === "string"
            ? parseFloat(crypto.quotes.USD.market_cap)
            : crypto.quotes.USD.market_cap;
        const volume24h =
          typeof crypto.quotes.USD.volume_24h === "string"
            ? parseFloat(crypto.quotes.USD.volume_24h)
      ...
      const projects = validCryptos.map((crypto: CryptoData) => {
        ...
        const price =
          typeof usdQuote.price === "string"
            ? parseFloat(usdQuote.price)
            : usdQuote.price;
        const marketCap =
          typeof usdQuote.market_cap === "string"
            ? parseFloat(usdQuote.market_cap)
            : usdQuote.market_cap;
        const volume24h =
          typeof usdQuote.volume_24h === "string"
            ? parseFloat(usdQuote.volume_24h)
            : usdQuote.volume_24h;
        const change24h =
          typeof usdQuote.percent_change_24h === "string"
            ? parseFloat(usdQuote.percent_change_24h)
            : usdQuote.percent_change_24h;

        const athPrice =
          usdQuote.ath_price !== undefined
            ? typeof usdQuote.ath_price === "string"
              ? parseFloat(usdQuote.ath_price)
              : usdQuote.ath_price
            : price * 1.5;

        const athChange =
          usdQuote.percent_from_price_ath !== undefined
            ? typeof usdQuote.percent_from_price_ath === "string"
              ? parseFloat(usdQuote.percent_from_price_ath)
              : usdQuote.percent_from_price_ath
            : -20;
        ...
        return {
          ...
          circulating_supply:
            crypto.circulating_supply !== undefined
              ? typeof crypto.circulating_supply === "string"
                ? parseFloat(crypto.circulating_supply)
                : crypto.circulating_supply
              : 0,
          max_supply:
            crypto.max_supply !== undefined
              ? typeof crypto.max_supply === "string"
                ? parseFloat(crypto.max_supply)
                : crypto.max_supply
              : undefined,
          ath: athPrice,
  ...
  calculateGemScore(project: CryptoProject) {
    ...
    if (
      project.ath_change_percentage > -30 &&
      project.price_change_percentage_24h > 10
    )
      score += 20;
    if (
      project.ath_change_percentage < -70 &&
      project.price_change_percentage_24h > 5
    )
      score += 15;
  ...
  async getOrCreateState() {
    let state = await this.prisma.cryptoGemState.findFirst({
      where: { id: 1 },
    });
    if (!state) {
      state = await this.prisma.cryptoGemState.create({
        data: {
          id: 1,
          currentPage: 1,
          maxPages: 40,
  ...
  async findGemsQuick() {
    ...
    console.log(
      `DEBUG: ${projects.length} cryptos récupérées sur la page ${currentPage}`
    );
    if (projects.length > 0) {
      console.log(
        "Exemple de projet brut:",
        JSON.stringify(projects[0], null, 2)
      );
    }
    const filtered = projects.filter(
      (project) =>
        project.market_cap < this.gemCriteria.maxMarketCap &&
        project.market_cap_rank <= this.gemCriteria.maxRank &&
        project.price_change_percentage_24h > this.gemCriteria.minPriceIncrease
    );
    console.log(
      `DEBUG: ${filtered.length} cryptos passent le filtre de base (market_cap, rank, price_change)`
    );
      const scored = filtered.map((project) => {
        const gemScore = this.calculateGemScore(project);
        console.log(`DEBUG: Score pour ${project.name} (${project.symbol}) : ${gemScore}`);
        return { ...project, gemScore };
      });
      const gems = scored
        .filter((project) => (project.gemScore || 0) >= 20)
        .sort((a, b) => (b.gemScore || 0) - (a.gemScore || 0))
        .slice(0, 20);
      console.log(`DEBUG: ${gems.length} cryptos ont un gemScore >= 20 (candidates)`);
    ...
    for (const gem of gems) {
      await this.prisma.cryptoGemProject.upsert({
        ...
        update: {
          ...
          priceChangePercentage24h: gem.price_change_percentage_24h,
          volume24h: gem.volume_24h,
        ...
        create: {
          ...
          name: gem.name,
          currentPrice: gem.current_price,
          marketCap: gem.market_cap,
          marketCapRank: gem.market_cap_rank,
          priceChangePercentage24h: gem.price_change_percentage_24h,
          volume24h: gem.volume_24h,
          totalVolume: gem.total_volume,
          circulatingSupply: gem.circulating_supply,
          maxSupply: gem.max_supply,
          ath: gem.ath,
          athChangePercentage: gem.ath_change_percentage,
          gemScore: gem.gemScore,
          needsSentimentAnalysis: (gem.gemScore || 0) >= 50,
          lastUpdated: new Date(),
        },
      });
    }
    ...
    console.log(
      `📄 Page ${currentPage} analysée → Prochaine: ${nextPage}${isNewCycle ? " (nouveau cycle)" : ""}`
    );
    if (isNewCycle) {
  ...
  async monitorProjects(projects: CryptoProject[]) {
    for (const project of projects) {
      if (project.price_change_percentage_24h > 50) {
        this.alerts.push({
          type: "price",
          message: `${project.name} (${project.symbol}) +${project.price_change_percentage_24h.toFixed(2)}% en 24h!`,
      ...
      if (volumeRatio > 0.2) {
        this.alerts.push({
          type: "volume",
          message: `Volume inhabituel pour ${project.name}: ${(volumeRatio * 100).toFixed(2)}%`,
          project: project.symbol,
          priority: "medium",
          timestamp: new Date(),
        });
      ...
      if (project.gemScore && project.gemScore > 70) {
        this.alerts.push({
          type: "gem",
          message: `Pépite détectée: ${project.name} (Score: ${project.gemScore})`,
          project: project.symbol,
          priority: "high",
          timestamp: new Date(),
        });
  ...
  async generateReport(gems: CryptoProject[]): Promise<CryptoGemReport> {
    const reportData: CryptoGemReport = {
      ...
      gems: gems.map((gem) => ({
        ...
        marketCap: gem.market_cap,
        priceChange24h: gem.price_change_percentage_24h,
        gemScore: gem.gemScore,
        sentiment: gem.socialSentiment,
        rank: gem.market_cap_rank,
      })),
      alerts: this.alerts,
      telegramSent: false,
    };
    ...
    for (const gem of gems) {
      await this.prisma.cryptoGemProject.upsert({
        ...
        update: {
          ...
          athChangePercentage: gem.ath_change_percentage,
          gemScore: gem.gemScore,
          sentimentScore: gem.socialSentiment?.score,
          sentimentMentions: gem.socialSentiment?.mentions,
          sentimentPositiveRatio: gem.socialSentiment?.positiveRatio,
          lastUpdated: new Date(),
  ...
  async saveAlertsToDB() {
    for (const alert of this.alerts) {
      await this.prisma.cryptoGemAlert.create({
        data: {
          type: alert.type,
  ...
  async cleanupOldData() {
    ...
    await this.prisma.cryptoGemAlert.deleteMany({
      where: { createdAt: { lt: oneDayAgo } },
    });
    await this.prisma.cryptoGemProject.deleteMany({
      where: {
        AND: [{ lastUpdated: { lt: oneWeekAgo } }, { gemScore: { lt: 30 } }],
      },
    });
    await this.prisma.cryptoGemProject.updateMany({
      where: { lastUpdated: { lt: new Date(Date.now() - 6 * 60 * 60 * 1000) } },
      data: { needsSentimentAnalysis: true },
    });
  }
  ...
  async getSystemStats() {
    ...
    const highScoreGems = await this.prisma.cryptoGemProject.count({
      where: { gemScore: { gte: 70 } },
    });
    const projectsNeedingAnalysis = await this.prisma.cryptoGemProject.count({
      where: { needsSentimentAnalysis: true },
    });

    // Calculer les statistiques du cycle
    const cycleProgress = (
      ((state.currentPage - 1) / state.maxPages) *
      100
    ).toFixed(1);
    const timesSinceLastCycle = state.lastCycleStart
      ? Math.floor((Date.now() - state.lastCycleStart.getTime()) / (1000 * 60))
      : 0;
  ...
  getGemCriteria() {
    return this.gemCriteria;
  }
  setGemCriteria(criteria: Partial<GemCriteria>) {
    this.gemCriteria = { ...this.gemCriteria, ...criteria };
  }
  setTelegramCooldown(hours: number) {
    this.telegramCooldownHours = Math.max(0.5, Math.min(72, hours));
  }
  getTelegramCooldown() {
    return this.telegramCooldownHours;
  }
}
