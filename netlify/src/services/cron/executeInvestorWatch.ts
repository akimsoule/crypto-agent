...
export async function executeInvestorWatch(prisma?: PrismaClient): Promise<CronExecutionResult> {
  ...
  try {
    ...
    // Récupérer l'historique des prix pour BTC, ETH, XRP (exemple: 48 dernières heures)
    async function fetchPriceHistory(symbol: string): Promise<number[]> {
      // À adapter selon ta source réelle (ici Coinpaprika)
      try {
        const res = await fetch(`https://api.coinpaprika.com/v1/tickers/${symbol}-usd/historical?start=${new Date(Date.now()-48*60*60*1000).toISOString()}&interval=1h`);
    ...
    console.log("📈 Récupération des historiques de prix pour les indicateurs techniques...");
    for (const sym of ["btc", "eth", "xrp"]) {
      priceHistories[sym] = await fetchPriceHistory(sym);
      console.log(`  ${sym.toUpperCase()}: ${priceHistories[sym].length} points de prix récupérés`);
    }

    // Mapping des gems enrichi - Tous les gems ont la structure de base
    const gems: GemWithIndicators[] = gemsFromDB.map((gem) => {
      ...
      // Ajouter les indicateurs uniquement pour BTC, ETH, XRP
      if (["btc", "eth", "xrp"].includes(symbolLower)) {
        const prices = priceHistories[symbolLower] || [];
        if (prices.length >= 10) { // Minimum de points pour calculer MACD
          const macd = computeMACD(prices);
          const envelope = computeEnvelope(prices);
          console.log(`  ✅ ${symbolLower.toUpperCase()}: Indicateurs calculés (MACD: ${macd?.macd?.toFixed(4) || 'N/A'}, Signal: ${macd?.signal?.toFixed(4) || 'N/A'}, Envelope SMA: ${envelope?.sma?.toFixed(2) || 'N/A'})`);
          return {
            ...baseGem,
            macd,
            envelope,
          ...
          console.log(`  ⚠️ ${symbolLower.toUpperCase()}: Pas assez de données historiques (${prices.length} points)`);
        }
      }
    ...
    // Log des gems avec indicateurs
    const gemsWithIndicators = gems.filter(g => g.macd !== undefined || g.envelope !== undefined);
    if (gemsWithIndicators.length > 0) {
      console.log(`📈 Gems avec indicateurs techniques: ${gemsWithIndicators.map(g => g.symbol).join(', ')}`);
      gemsWithIndicators.forEach(gem => {
        if (gem.macd) {
          console.log(`  ${gem.symbol}: MACD=${gem.macd.macd?.toFixed(4)}, Signal=${gem.macd.signal?.toFixed(4)}`);
        }
        if (gem.envelope) {
          console.log(`  ${gem.symbol}: Envelope SMA=${gem.envelope.sma?.toFixed(2)}, Upper=${gem.envelope.upper?.toFixed(2)}, Lower=${gem.envelope.lower?.toFixed(2)}`);
        }
      });
    } else {
      console.log("⚠️ Aucun gem avec indicateurs techniques trouvé");
    }
    ...
    const investorPromises = activeInvestors.map(async (investorProfile) => {
      ...
      try {
        // Filtrer les indicateurs selon le profil de l'investisseur
        ...
        console.log(`  📊 ${investorProfile.name} (${type}) - Utilise indicateurs: ${usesIndicators ? 'OUI' : 'NON'} (technicalWeight: ${investorProfile.technicalWeight})`);
        
        let gemsForInvestor: (GemBase | GemWithIndicators)[];
        if (usesIndicators) {
          // Investisseur technique : garde tous les gems avec leurs indicateurs
          gemsForInvestor = gems;
          const gemsWithIndicators = gems.filter(gem => gem.macd !== undefined || gem.envelope !== undefined).length;
          console.log(`    💡 Gems avec indicateurs fournis: ${gemsWithIndicators}/${gems.length}`);
        } else {
          // Investisseur non-technique : retire les indicateurs des gems
          gemsForInvestor = gems.map(gem => {
            const { macd, envelope, ...rest } = gem;
            void macd;
            void envelope;
            return rest;
          });
          console.log(`    🔒 Indicateurs masqués pour cet investisseur`);
        }
